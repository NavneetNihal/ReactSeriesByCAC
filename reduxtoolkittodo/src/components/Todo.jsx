...
import { removeTodo, updateTodo } from '../features/todo/TodoSlice' // ← Added updateTodo

function Todos() {
  const todos = useSelector(state => state.todos)
  const dispatch = useDispatch()

  // ← NEW: These 2 lines only
      ...
      <ul className="list-none">
        {todos.map((todo) => (
          <li
            ...
            key={todo.id}
          >
            {/* ← THE ONLY CHANGE: Conditional content */}
            {editingId === todo.id ? (
              // EDIT MODE
              <>
                <input
                  ...
                  className="flex-1 bg-gray-700 text-white px-2 py-1 rounded mr-2"
                  autoFocus
                />
                <button
                  onClick={() => {
